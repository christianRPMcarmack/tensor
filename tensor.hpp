...
private:
	/* ---------------- PROPERTIES -------------- */
	std::vector<size_t> mShape; // shape of array i.e. 2x3 matrix
	std::vector<T> mData; // vector where the data is stored
	std::vector<T> mSkipCoeffs; // size of slices to skip
...
public:
	...
	// int constructor i.e. tensor<T> x(1,2);
	template <typename... dimType>
	tensor(dimType ... _shape) { sizeArray({ std::forward<dimType>(_shape)... }); }

	// move constructor
	tensor(tensor<T>&& rhs) : mShape(std::move(rhs.mShape())), mData(std::move(rhs.mData)), mSkipCoeffs(std::move(rhs.mSkipCoeffs)) {}

	// copy constructor
	tensor(const tensor<T>& rhs) : mShape(rhs.mShape), mData(rhs.mData), mSkipCoeffs(rhs.mSkipCoeffs) {}
...
// linear interpolation returns interpolated xData to match yData
template<typename T>
tensor<T> linterp(tensor<T>& xData, tensor<T>& yData) {
	tensor<T> out(yData.size());
